...
export default function RoundProgressPanel() {
  ...
  const submittedUserUuids = useRelayDrawingStore(
    (state) => state.submittedUserUuids,
  );
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid gap-4">
        {participants.map((participant) => {
          const hasSubmitted = submittedUserUuids.includes(
          ...
          return (
            <div
              ...
              className={cn(
                ...
                hasSubmitted &&
                  "border-relay-accent-strong bg-relay-accent/40",
              )}
            >
              <span
                className={cn(
                  "grid size-7 place-items-center rounded-full caption-b",
                  hasSubmitted
                    ? "bg-relay-accent-strong text-fg-inverse"
                    : "bg-relay-paper text-relay-ink",
                )}
              >
                {hasSubmitted ? (
