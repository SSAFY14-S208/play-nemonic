...
| k6 실행 가이드 | `backend/docs/performance/k6-실행-가이드.md` |
...
| --- | ---: | --- | --- | --- | --- |
| 운세 생성 | 5 | 20s | 5s | 5s | GMS 로컬 stub, MinIO 로컬 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 운세 생성 | 465 | 15.45 | 162.66ms | 204.33ms | 243.67ms | 0.00% |
| 문의 답변 | 967 | 32.09 | 13.18ms | 19.53ms | 35.20ms | 0.00% |

이 k6 결과는 트랜잭션 커넥션 점유 before/after를 직접 보여주는 값은 아닙니다. 커넥션 점유 before/after는 위 synthetic capacity model로 설명하고, k6는 리팩토링 이후 실제 HTTP 경로가 로컬 통제 환경에서 실패 없이 안정적으로 처리되는지 확인하는 용도입니다.
