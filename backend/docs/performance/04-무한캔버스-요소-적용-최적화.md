...
| 구분 | 경로 |
| --- | --- |
| 최적화 문서 | `backend/docs/performance/04-무한캔버스-요소-적용-최적화.md` |
...
| 관련 k6 가이드 | `backend/docs/performance/06-k6-부하-테스트-실행-가이드.md` |
| 그래프 assets | `backend/docs/performance/assets/infinite-canvas-operation-apply-p95-ko.svg`, `backend/docs/performance/assets/infinite-canvas-operation-lookup-steps-ko.svg` |
...
| 관련 무한캔버스 k6 | `backend/scripts/k6/infinite-canvas-active-room-load.js` | 트러블 슈팅 3의 활성 방 조회 API 검증용 |
...
| 참고 가능한 관련 k6 | `backend/scripts/k6/infinite-canvas-active-room-load.js` |
