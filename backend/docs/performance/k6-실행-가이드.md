...
| 트러블 슈팅 4 | 무한캔버스 요소 적용 로직 최적화 | 직접 k6 없음 | synthetic benchmark로 operation apply p95 측정 |
...
터미널 캡처도 가능하지만, 포트폴리오에는 각 트러블 슈팅 문서의 `사용한 k6` 섹션과 생성된 Markdown 요약의 p95/RPS 표를 함께 사용하는 편이 더 안정적입니다.

2026-06-01 로컬 통제 환경에서 실행한 결과 요약은 각 트러블 슈팅 문서 내부에 분산 정리했습니다. 원본 Markdown/JSON은 `backend/docs/performance/k6-results`에 남겨둡니다.
