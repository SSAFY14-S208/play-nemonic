...
터미널 캡처와 before / after 표 정리 기준은 `backend/docs/performance/k6-상세-캡처-가이드.md`에 따로 정리했습니다.
...
| --- | --- | --- | --- |
| 트러블 슈팅 1 | 운세 생성 외부 I/O 트랜잭션 분리 | `backend/docs/performance/01-운세-생성-외부-io-트랜잭션-분리/k6/01-운세-생성-k6.js` | 운세 생성 p95, 실패율, Hikari pending connection |
| 트러블 슈팅 2 | 문의 답변 SMTP 외부 I/O 트랜잭션 분리 | `backend/docs/performance/02-문의-답변-메일-io-트랜잭션-분리/k6/02-문의-답변-k6.js` | 문의 답변 p95, 실패율, Hikari pending connection |
| 트러블 슈팅 3 | 갤러리 목록 조회 쿼리 최적화 | `backend/docs/performance/03-갤러리-목록-조회-쿼리-최적화/k6/03-갤러리-목록-조회-k6.js` | `http_req_duration p95`, RPS |
| 트러블 슈팅 4 | 무한캔버스 활성 방 목록 인덱스 최적화 | `backend/docs/performance/04-무한캔버스-조회-payload-최적화/k6/04-무한캔버스-활성-방-목록-k6.js` | backoffice 목록 p95, 실패율 |
| 트러블 슈팅 5 | 무한캔버스 요소 적용 로직 최적화 | 직접 k6 없음 | synthetic benchmark로 operation apply p95 측정 |
| 트러블 슈팅 6 | 릴레이/플립북 활성 방 인덱스 최적화 | `backend/docs/performance/06-릴레이-플립북-활성-방-인덱스-최적화/k6/` | backoffice 목록 p95, 실패율 |
| 트러블 슈팅 7 | 커뮤니티 메모 조회 인덱스 최적화 | `backend/docs/performance/07-커뮤니티-메모-조회-인덱스-최적화/k6/` | 공개/관리자 목록 p95, 실패율 |
...
운세 생성과 문의 답변은 같은 트랜잭션 경계 축소 기법을 사용하지만, 병목 외부 시스템과 장애 시나리오가 달라서 각각 별도 트러블 슈팅으로 실행하고 캡처합니다.
...
터미널에는 `http_req_duration`, p90, p95, p99, 실패율, RPS가 함께 출력됩니다. 포트폴리오에는 터미널 캡처와 각 트러블 슈팅 문서의 `사용한 k6` 섹션, 생성된 Markdown 요약의 p95/RPS 표를 같이 사용하면 좋습니다.

2026-06-01 로컬 통제 환경에서 실행한 결과 요약은 각 트러블 슈팅 문서 내부에 분산 정리했습니다. 원본 Markdown/JSON은 `각 트러블슈팅 폴더의 k6/results`에 남겨둡니다.
