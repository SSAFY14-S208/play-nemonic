...
| `relay_finalization_immediate_trigger_failed` | `ALL_PARTS_COMPLETED` 직후 비동기 finalization trigger 작업 시작 또는 실행 wrapper가 실패한 때 | `room_id`, `stage`, `error` | 있음 | `stage=task_submit`은 executor가 작업을 받지 못한 경우, `stage=async_run`은 비동기 wrapper 실행 중 예외가 바깥까지 나온 경우다. 제출/timeout/disconnect grace 흐름은 실패시키지 않고 scheduler retry가 복구한다. |
| `relay_finalization_failed` | 최종 결과물 생성 과정에서 실패한 때 | `room_id`, `stage`, `artifact_id`, `attempt_id`, `operation`, `retry_count`, `max_retry_count` | 있음 | 최종화 실패 단계별 추적 로그다. `stage`는 `process`, `compose`, `minio_upload`, `artifact_meta`, `db_save`, `redis_update` 등이 들어간다. 기본 정책은 10초 간격으로 최대 60회 재시도하고, 60회째 실패하면 방을 `CLOSED`로 전환한다. |
