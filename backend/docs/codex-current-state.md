...
- Community memo creation now uses `POST /api/v1/community/memos` with required `Anonymous-User-UUID`, supports `sourceType=DIRECT` and `sourceType=GALLERY`, requires distinct confirmed `COMMUNITY` `originalFileId` and `thumbnailFileId`, links GALLERY posts to an owned active gallery artifact only for source attribution, creates transparent PNG display derivatives under `community/memos/{memoId}/body.png` and `community/memos/{memoId}/thumbnail.png`, runs pre-publication moderation with derivative public URLs before insert, stores derivative object keys in `community_memo.body_image_url` and `community_memo.thumbnail_image_url`, and applies 50-visible-memo FIFO soft deletion with `deleted_reason=expired`.
- Community memo image derivatives preserve the original uploaded `file_upload` objects and artifact result objects. The first implementation uses local white-key processing only, with `nemonic.community.memo.image.white-threshold` defaulting to `245`; every white-like pixel is made transparent, including white regions inside closed shapes. AI/GMS/background-segmentation APIs remain a follow-up candidate for complex photo cutouts, and no DB migration is needed because existing memo image columns store the derivative object keys.
