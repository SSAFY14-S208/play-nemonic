...
- Community direct memo creation now uses `POST /api/v1/community/memos` with required `Anonymous-User-UUID`, supports only `sourceType=DIRECT`, requires distinct confirmed `COMMUNITY` `originalFileId` and `thumbnailFileId`, runs pre-publication moderation before insert, stores the final original object key in `community_memo.body_image_url` and thumbnail object key in `community_memo.thumbnail_image_url`, and reuses detail response mapping; GALLERY creation and FIFO cleanup remain follow-up work.
