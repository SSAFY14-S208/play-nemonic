...
- Relay drawing submissions now advance the Redis room state from `FACE` to `BODY` and `BODY` to `LEGS` when every assignment in the current part is `SUBMITTED` or `AUTO_SUBMITTED`; completing `LEGS` moves the room to `FINALIZING`, emits `ALL_PARTS_COMPLETED`, and immediately triggers one finalization attempt while keeping the scheduler as the retry/recovery path.
