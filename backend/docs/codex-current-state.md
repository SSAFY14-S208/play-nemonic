...
- Relay finalization async executor values are configurable under `nemonic.relay.finalization.async.*`; defaults are core pool size `2`, max pool size `4`, queue capacity `100`, and thread prefix `relay-finalization-`.
