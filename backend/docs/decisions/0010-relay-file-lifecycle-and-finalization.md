...
`canvasIndex`. By default, `nemonic.relay.finalization.overlap-height=0` keeps
the previous vertical composition. If the setting is positive, the backend
overlaps `FACE`/`BODY` and `BODY`/`LEGS` by the configured pixel height, clamps
excessive overlap so the result remains valid, and renders the overlap with
layer priority `FACE > BODY > LEGS`. This changes only final result rendering;
the game still progresses in `FACE -> BODY -> LEGS` order.
