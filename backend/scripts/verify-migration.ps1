...
try {
    ...
    throw "Docker is required for PostgreSQL migration verification. Start Docker Desktop, then rerun this script."
}
...
try {
    ...
    Write-Host "Running Gradle task: migrationTest"
    & $gradlePath "--no-daemon" "-Dmaven.repo.local=$mavenRepo" "-Duser.home=$toolHome" "migrationTest"
