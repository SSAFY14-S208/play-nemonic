...
/**
 * 커뮤니티 메모 이미지 object key를 브라우저에서 접근 가능한 public URL로 변환합니다.
 */
@Component
public class CommunityMemoImageUrlResolver {
    ...
    public String resolve(String objectKey) {
        ...
        if (!StringUtils.hasText(publicUrl) || !StringUtils.hasText(bucket)
            ...
            log.warn("커뮤니티 메모 이미지 URL을 생성할 수 없습니다. publicUrl={}, bucket={}, objectKey={}", publicUrl, bucket,
                objectKey);
        ...
        try {
            ...
            log.warn("커뮤니티 메모 이미지 URL 생성 중 오류가 발생했습니다. objectKey={}", objectKey, e);
